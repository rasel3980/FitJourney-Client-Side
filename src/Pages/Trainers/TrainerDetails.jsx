...
const TrainerDetails = () => {
  ...
  const { id } = useParams(); // Use the trainer's ID from the route params
  ...
  // Function to handle navigating to the trainer booking page for the selected slot
  const handleSlotClick = (slot) => {
    // Redirect to the trainer booking page with selected trainer and slot information
    navigate(`/trainerBooked/${id}`, {
      state: { selectedSlot: slot },  // Pass slot details via state
    });
  };
                    ...
                    <div key={dayObj.day} className="w-full text-center p-4 bg-white rounded-lg border border-red-800 shadow-md">
                      ...
                      <ul className="space-y-2 mt-2">
                        {dayObj.timeSlots.map((slot) => (
                          ...
                          >
                            <button
                              onClick={() => handleSlotClick(slot)} // Redirect to the booking page
